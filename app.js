(() => {
  ...
  if(image){
    const pages=[
      {img:'/assets/issues/001/01-cover.webp',title:'2027: cuando la IA deja de ser un chatbot',desc:'La tesis de apertura: el siguiente salto no es sólo un modelo más grande, sino un sistema capaz de decidir cuándo y cómo utilizar inteligencia.',chips:['2027','Adaptive AI','Organisms']},
      {img:'/assets/issues/001/02-modelo-organismo.webp',title:'Del modelo al organismo',desc:'El modelo deja de ser el producto completo. Memoria, planificación, herramientas, presupuesto y mundo real empiezan a determinar la capacidad final.',chips:['Harness','Memory','Planning']},
      {img:'/assets/issues/001/03-agentes-inteligencia.webp',title:'Más agentes ≠ más inteligencia',desc:'Un swarm puede duplicar trabajo, converger demasiado pronto y quemar recursos. La coordinación, los incentivos y la evidencia importan más que el número de agentes.',chips:['Swarms','Coordination','Evidence']},
      {img:'/assets/issues/001/04-mosca-pista.webp',title:'La mosca nos da una pista',desc:'La biología sugiere una arquitectura: circuitos locales para lo frecuente y un “córtex excepcional” sólo cuando la situación realmente lo necesita.',chips:['Connectome','Reflex Mesh','Bio-inspired']},
      {img:'/assets/issues/001/05-pensar-mejor.webp',title:'Pensar menos. Pensar mejor.',desc:'REFLEX, THINK y DELIBERATE convierten el cómputo en un presupuesto. No todas las decisiones merecen el mismo nivel de razonamiento.',chips:['Metabolism','SLM','Efficiency']},
      {img:'/assets/issues/001/06-harness.webp',title:'El harness es el nuevo modelo',desc:'El sistema puede compilar un runtime específico para cada tarea: memoria, tools, protocolo, modelo, plan y subagentes temporales.',chips:['JIT Harness','Runtime','Ephemeral Agents']},
      {img:'/assets/issues/001/07-razonamiento-software.webp',title:'Del razonamiento al software',desc:'Una solución costosa se extrae, se valida, pasa canary y puede convertirse en un procedimiento local reutilizable. Si degrada, se pone en cuarentena.',chips:['Skills','Canary','Immune System']},
      {img:'/assets/issues/001/08-reality-bridge.webp',title:'Reality Bridge',desc:'La IA propone; la evidencia decide. Archivos, líneas y fragmentos reales forman un paquete verificable antes de aceptar una hipótesis.',chips:['Reality','Grounding','Verification']},
    ...
    addEventListener('popstate',()=>{const n=parseInt(new URL(location.href).searchParams.get('page')||'1',10);show(Math.max(0,Math.min(9,n-1)),false)});
    const requested=parseInt(new URL(location.href).searchParams.get('page')||'1',10);show(Math.max(0,Math.min(9,requested-1)),false);
    const lb=document.getElementById('lightbox'), lbi=document.getElementById('lightbox-img');
    const openLightbox=()=>{lbi.src=pages[current].img;lb.classList.add('open');lb.setAttribute('aria-hidden','false')};
